...
def dipole_amplitude(b, r, x, theta, A):
    """
...
def sigma_dip(r, x, theta, b=None, A=1, dip_theory='GBW'):
    ...
    dip_theory : string, optional
        Determines which dipole theory model is used. The default is 'GBW' for only one proton.
        Then this does not need impact parameter. If set to 'other', the model includes whole
        nucleus and needs impact parameter.
    ...
    if dip_theory == 'GBW':
        ...
        return 2 * dipole_amplitude(b, r, x, theta, A=A)
...
def sigma_T(x, Q2, theta, Zf, mf, A=1, dip_theory='GBW'):
    ...
    if dip_theory == 'GBW':
        ...
        # The function for integral. \int dr^2 = \int dr r \int dtheta = \int dr r 2pi
        transverse = lambda b, r, z: np.sum(Psi_T_abs_square(r, z) * sigma_dip(r, x, theta, b, A=A, dip_theory=dip_theory)) * r * b * 2 * np.pi
...
def sigma_L(x, Q2, theta, Zf, mf, A=1, dip_theory='GBW'):
    ...
    if dip_theory == 'GBW':
        ...
        # The function for integral. \int dr^2 = \int dr r \int dtheta = \int dr r 2pi
        longitudal = lambda b, r, z: z**2 * (1 - z)**2 * np.sum(Psi_L_abs_square(r, z) * sigma_dip(r, x, theta, b, A=A, dip_theory=dip_theory)) * r * b * 2 * np.pi
...
def F2(x, Q2, sigma0, lambd, Q02, A=1, dip_theory='GBW'):
    ...
    """
    if dip_theory != 'GBW': normalize_ws(A)
    x0 = 2.24 * 10**(-4)
